...
def run_s23_bull_call_vertical_slice() -> Any:
    case = build_s23_bull_call_vertical_case()
    stages = (
...
def build_s23_bull_call_vertical_case() -> S23VerticalSliceCase:
    rule = load_strategy_rule(ROOT / "config" / "strategies" / "options_sell" / "nifty" / "S23_NIFTY_OP_SELL_WK_DIFF_2D_3D")
    ...
    runtime_input = TFISRuntimeInput(
        ...
        provenance={"source": "phase3d_m3_synthetic"},
        ...
        resolved_configuration_hash="phase3d-m3-s23-bull-call-synthetic-v1",
    )
    return S23VerticalSliceCase(runtime_input, rule, market, runtime_values, chain)
...
def _decision(context: Mapping[str, Any]) -> OfflineStageResult:
    ...
    decision = TFISDecision(
        ...
        intermediate_calculation_evidence={
            "pipeline": "phase3d_m3_s23_vertical",
        ...
        compatibility_payload={"vertical_slice": "S23_BULL_CALL"},
        strategy_family_id="S23",
        strategy_definition_id="S23_NIFTY_OP_SELL_WK_DIFF_2D_3D",
...
def _legacy_comparison(context: Mapping[str, Any]) -> OfflineStageResult:
    ...
    compared = {
        ...
        "selected_strike": {"legacy": contract.selected_contract.strike, "vertical": decision.selected_instrument.strike, "classification": "MATCH"},
...
def _entry_input(case: S23VerticalSliceCase, selected: TFISContractIdentity, legacy_entry: Any, gme: Any | None) -> EntryEngineInput:
    ...
    identity = StrategyEvaluationIdentity.deterministic(strategy_instance_id="S23_NIFTY_ACCOUNT_A_PAPER", strategy_definition_id="S23_NIFTY_OP_SELL_WK_DIFF_2D_3D", strategy_version="1.0.0", trading_date=ts.date(), evaluation_timestamp=ts, evaluation_sequence=1, trigger_type="phase3d_m3", configuration_hash=case.runtime_input.resolved_configuration_hash)
    ...
    return EntryEngineInput(
        ...
        resolved_branch=EntryResolvedBranch(EntryMarketBias.BULLISH, S23_BULL_CALL_BRANCH, TFISProductType.OPTION_SELLING, EntryInstrumentType.SELECTED_OPTION_CONTRACT, EntryOptionRight.CALL, EntryPositionIntent.SHORT_OPTION, TFISExecutionSide.SELL, EntryTriggerDirection.PRICE_AT_OR_BELOW),
...
def _packet(case: S23VerticalSliceCase, context: Mapping[str, Any]) -> TFISDecisionEvidencePacket:
    ...
    return TFISDecisionEvidencePacket(
        identity=IdentityEvidence(SCHEMA_VERSION, "SYNTHETIC_GOLDEN:S23:BULL_CALL:PHASE3D_M3", case.runtime_input.evaluation_id, "S23_NIFTY_ACCOUNT_A_PAPER", case.strategy_rule.unique_code, case.strategy_rule.unique_code, "1.0.0", case.runtime_input.resolved_configuration_hash, ts.date(), ts, ts, ts + timedelta(seconds=1)),
        session=SessionEvidence("NSE", Segment.OPTIONS_SELL, "Asia/Kolkata", TimeWindowEvidence(time(9, 15), time(15, 30), time(9, 24, 59), time(9, 29, 59), "PHASE3D_M3_SYNTHETIC", "S23 Bull Call vertical slice.")),
        ...
        monthly_status=MonthlyStatusEvidence(MonthlyStatus.BULL, MonthlyStatus.BULL, "synthetic monthly status", _pv(24900), _pv(24000), _pv(24680), _pv(23840), _pv(24550), _pv(23820), _pv(24750), _pv(23950), _pv("0.75"), _pv("0.75"), _pv("0.15"), "NO_TRANSITION", "Synthetic BULL status.", "VALID"),
        ...
        calculated_decision=CalculatedDecisionEvidence(_pv(context["effective_entry"].effective_entry.value), (_pv(context["target"].calculated_value),), _pv(context["msl"].calculated_value), _not_applicable(), _not_applicable(), _pv(case.runtime_input.lots), _pv(case.runtime_input.quantity), TFISDirection.SHORT, TFISExecutionSide.SELL, TFISTradeResult.TRADE, "S23 Bull Call vertical slice expected TRADE."),
        audit=AuditEvidence((("entry", S23VerticalEntryPolicy.policy_key), ("gap_missed_entry", S23_BACKTEST_LOW_POLICY_KEY)), ("AB16-ENTRY", "AB16-CONTRACT", "AB16-TARGET", "AB16-MSL"), (("entry", case.strategy_rule.entry_formula), ("target", case.strategy_rule.target_formula), ("stoploss", case.strategy_rule.stoploss_formula)), (("base_entry", _pv(context["base_entry"].base_entry.value)), ("effective_entry", _pv(context["effective_entry"].effective_entry.value))), (), (EvidenceProvenance.SYNTHETIC,), {"label": "PHASE3D_M3_S23_BULL_CALL"}),
        entry=EntryBusinessEngineFragment("entry", S23VerticalEntryPolicy.policy_key, "phase3d_m3", "OPTION_SELLING", S23_BULL_CALL_BRANCH, selected.symbol, case.strategy_rule.entry_formula, (("legacy_entry_value", _pv(context["base_entry"].base_entry.value)),), (), (), _pv(context["base_entry"].base_entry.value), {"missed_entry": context["gap_missed_entry"].missed_entry.status.value}, _pv(context["effective_entry"].effective_entry.value), context["effective_entry"].effective_entry.source.value, {"trigger_direction": context["effective_entry"].effective_entry.trigger_condition.trigger_direction.value}, (), (), (), "VALID", {"source": "phase3d_m3"}, context["effective_entry"].deterministic_hash),
    )
...
def _candidate(identity: TFISContractIdentity, ts: datetime) -> OptionChainCandidateEvidence:
    return OptionChainCandidateEvidence(identity, _pv(identity.strike), OptionType.CALL, identity.expiry, _pv(identity.metadata["ltp"]), _pv(Decimal(str(identity.metadata["ltp"])) - Decimal("1")), _pv(Decimal(str(identity.metadata["ltp"])) + Decimal("1")), _pv(identity.metadata["oi"]), ts, "VALID", Decimal("0"))
