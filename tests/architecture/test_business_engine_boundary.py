...
def test_business_engine_contract_has_no_runtime_broker_or_legacy_imports() -> None:
    source = BUSINESS_ENGINE_SOURCE.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(BUSINESS_ENGINE_SOURCE))
    imports = []
...
def test_business_engine_contract_and_catalog_are_strategy_neutral() -> None:
    ...
    for path in (BUSINESS_ENGINE_SOURCE, CATALOG_SOURCE):
