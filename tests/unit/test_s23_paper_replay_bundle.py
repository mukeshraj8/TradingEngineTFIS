...
def test_valid_order_planned_session_creates_valid_bundle(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    session_dir = _write_snapshot(_planned_snapshot(), tmp_path, "planned-session")

    manifest_path = manager.create_bundle(
        session_dir,
        created_at=_ts(9, 31, 0),
        source_artifact_root=tmp_path / "paper_sessions",
    )

    assert manifest_path.exists()
    manifest = _read_json(manifest_path)
    assert manifest["terminal_state"] == "ORDER_PLANNED"
    assert manifest["session_id"] == "planned-session"

    validation = manager.validate_bundle(session_dir)
    assert validation.is_valid is True
    assert validation.terminal_state is PaperSessionState.ORDER_PLANNED


def test_valid_no_trade_session_creates_valid_bundle(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    ...
    assert validation.is_valid is True
    assert validation.terminal_state is PaperSessionState.NO_TRADE
    assert validation.terminal_reason_code == "monthly_status_unknown"


def test_valid_aborted_session_creates_valid_bundle(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    ...
    assert validation.is_valid is True
    assert validation.terminal_state is PaperSessionState.ABORTED
    assert validation.terminal_reason_code == "manual_operator_abort"


def test_missing_artifact_fails_validation(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    session_dir = _write_snapshot(_planned_snapshot(), tmp_path, "missing-artifact-session")
    manager.create_bundle(session_dir, created_at=_ts(9, 31, 0))

    (session_dir / "paper_order_plan.json").unlink()
    validation = manager.validate_bundle(session_dir)

    assert validation.is_valid is False
    assert "missing_required_artifact:paper_order_plan.json" in validation.errors


def test_modified_artifact_causes_hash_mismatch(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    session_dir = _write_snapshot(_planned_snapshot(), tmp_path, "hash-mismatch-session")
    manager.create_bundle(session_dir, created_at=_ts(9, 31, 0))

    payload = _read_json(session_dir / "paper_order_plan.json")
    payload["selected_contract_symbol"] = "NIFTY_20260528_22500_PE"
    ...
    assert validation.is_valid is False
    assert "hash_mismatch:paper_order_plan.json" in validation.errors


def test_terminal_mismatch_fails_validation(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    session_dir = _write_snapshot(_planned_snapshot(), tmp_path, "terminal-mismatch-session")
    manifest_path = manager.create_bundle(session_dir, created_at=_ts(9, 31, 0))

    manifest = _read_json(manifest_path)
    manifest["terminal_state"] = "NO_TRADE"
    ...
    assert validation.is_valid is False
    assert "terminal_state_mismatch" in validation.errors


def test_bundle_summary_is_deterministic(tmp_path: Path) -> None:
    manager = S23PaperReplayBundleManager()
    session_dir = _write_snapshot(_aborted_snapshot(), tmp_path, "summary-session")
    manager.create_bundle(session_dir, created_at=_ts(9, 31, 0))

    summary_a = manager.summarize_bundle(session_dir)
    summary_b = manager.summarize_bundle(session_dir)

    assert summary_a == summary_b
    assert summary_a.bundle_valid is True
    assert summary_a.terminal_reason_code == "manual_operator_abort"
    assert summary_a.audit_transition_count >= 1
