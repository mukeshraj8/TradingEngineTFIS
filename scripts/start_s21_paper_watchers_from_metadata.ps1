...
function Start-TfisS21Watcher {
    ...
    $stdoutPath = Join-Path $logDir "s21_paper_watch_${Mode}_${safeLabel}_${stamp}.out.log"
    $stderrPath = Join-Path $logDir "s21_paper_watch_${Mode}_${safeLabel}_${stamp}.err.log"
    $launchScriptPath = Join-Path $logDir "s21_paper_watch_${Mode}_${safeLabel}_${stamp}.ps1"
    ...
    $watchArgsLiteral = @(
        ...
        "'--process-lock-root'"
        Quote-TfisPowerShellLiteral "tmp/process_locks/s21_paper_watch"
        "'--s23-artifact-root'"
        Quote-TfisPowerShellLiteral "data/strategies/S21/fyers_morning_supervised_decision"
    )
...
`$Host.UI.RawUI.WindowTitle = 'TFIS S21 Paper Watch - $Mode'
Write-Host '============================================================'
Write-Host 'TFIS S21 PAPER WATCHER'
...
Write-Host '============================================================'
Write-Host "TFIS S21 paper watcher exited with code `$tfisWatchExitCode."
...
if (-not (Test-Path $dayRoot)) {
    throw "No TFIS S21 artifact day directory found for $($effectiveSessionDate.ToString('yyyy-MM-dd')): $dayRoot"
}
...
$started = foreach ($target in $watchTargets) {
    Start-TfisS21Watcher -Mode $target.Mode -Directory $target.Directory
}
