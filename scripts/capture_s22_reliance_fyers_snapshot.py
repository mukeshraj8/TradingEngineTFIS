...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Capture a read-only FYERS S22 RELIANCE metadata snapshot.")
    ...
    intended_requests = [
        ...
        "fetch_historical_candles:NSE:RELIANCE-EQ:daily",
        "fetch_option_chain:NSE:RELIANCE-EQ:near_monthly",
        "fetch_option_chain:NSE:RELIANCE-EQ:next_monthly",
    ]
    ...
    reliance_records = tuple(record for record in all_records if record.underlying == "RELIANCE" or "RELIANCE" in record.source_symbol.upper())
    expiry_classification = classify_monthly_expiries(reliance_records, underlying="RELIANCE", as_of=captured_at.date())

    completed_history_to = _last_completed_weekday(captured_at.date())
    ...
    for expiry in (expiry_classification.near_monthly_expiry, expiry_classification.next_monthly_expiry):
        ...
        option_chains.append(
            adapter.fetch_option_chain(
                ...
                instrument_records=reliance_records,
            )
        )

    snapshot_id = f"s22-reliance-fyers-{captured_at.strftime('%Y%m%dT%H%M%S%z')}"
    output_dir = args.artifact_root / captured_at.date().isoformat() / snapshot_id
    output_dir.mkdir(parents=True, exist_ok=False)

    snapshot = {
        "schema_version": "s22.reliance.fyers_read_only_snapshot.v1",
        ...
        "symbol_master": {
            ...
            "reliance_record_count": len(reliance_records),
            "reliance_records": [record.to_dict() for record in reliance_records],
        },
    ...
    required_gaps = _required_gaps(reliance_records, expiry_classification, history, option_chains)
    summary = {
...
def _required_gaps(records, expiry_classification, history, option_chains) -> list[str]:
    gaps: list[str] = []
    if not records:
        gaps.append("RELIANCE_SYMBOL_MASTER_RECORDS_MISSING")
    option_records = [record for record in records if record.option_type in {"CALL", "PUT"}]
    if not option_records:
        gaps.append("RELIANCE_OPTION_CONTRACTS_MISSING")
    if not any(record.lot_size for record in option_records):
        gaps.append("RELIANCE_OPTION_LOT_SIZE_MISSING")
    if not any(record.tick_size for record in option_records):
        gaps.append("RELIANCE_OPTION_TICK_SIZE_MISSING")
    ...
    if history.status != FyersReadOnlyStatus.SUCCESS or not history.payload.candles:
        gaps.append("RELIANCE_COMPLETED_DAILY_HISTORY_MISSING")
    if not option_chains:
        gaps.append("RELIANCE_OPTION_CHAIN_MISSING")
    if any(result.status != FyersReadOnlyStatus.SUCCESS for result in option_chains):
        gaps.append("RELIANCE_OPTION_CHAIN_UNAVAILABLE")
    return gaps
