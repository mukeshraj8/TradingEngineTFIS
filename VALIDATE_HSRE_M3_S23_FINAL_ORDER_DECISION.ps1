...
Run-Step "focused M3 and no-lookahead tests" {
    & $Python -m pytest tests\unit\test_hsre_s23_final_order_decision.py -q
}

Run-Step "ORPT LOW authority tests" {
    & $Python -m pytest tests\unit\test_s23_entry_missed.py -q
}

Run-Step "real Jan-3 M3 decision and deterministic hash" {
    & $Python -m pytest tests\integration\test_hsre_s23_final_order_decision_real_data.py -q -s
}
...
Write-Host "HSRE M3 validation completed."
