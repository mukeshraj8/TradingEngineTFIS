...
| `.\.venv\Scripts\python.exe scripts\serve_operator_dashboard.py --output-root tmp\operator_dashboard --port 8765 --skip-build` | Serve the operator dashboard locally and auto-rebuild stale dashboard pages on normal page requests. | After `build_operator_dashboard.py` or after the TFIS reset script already rebuilt the dashboard. | Open `http://127.0.0.1:8765/index.html`; pages show a built-at freshness strip, and stale pages rebuild after the configured `--auto-rebuild-seconds` interval. | Reads local artifacts only. Use `--disable-auto-rebuild` only when you intentionally want static serving with no request-time rebuild. |
| `powershell -ExecutionPolicy Bypass -File scripts\reset_tfis_dashboard_and_watchers.ps1 -MorningStartup` | Start TFIS as one application: build/serve the dashboard, prepare broker runtime auth once per configured provider, run configured morning strategy wrappers with refresh skipped, and start one shared lifecycle supervisor. | Normal scheduled morning startup path. The Windows task should be `TFIS Morning Startup`, enabled for weekdays at `09:08`. | Console/task logs should show provider auth preparation, dashboard URL on `127.0.0.1:8765`, configured wrappers launched sequentially, and shared supervisor launch. | Does not automatically stop existing TFIS runtime processes. The separate S21/S23 morning scheduled tasks should stay disabled to avoid auth races. |
