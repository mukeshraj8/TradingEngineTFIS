...
| `.\.venv\Scripts\python.exe scripts\show_tfis_live_money_boundary_status.py` | Show the explicit live-money execution/reconciliation boundary and required gates before live order routing can be considered. | During readiness review or before any discussion of enabling live-money mode. | Output should show `status=BLOCKED_FOR_LIVE_MONEY`, `live_money_ready=false`, and `order_routing_enabled=false` until a separate live execution layer is implemented and approved. | Read-only. This does not place orders; it documents that live routing remains blocked by design. |
