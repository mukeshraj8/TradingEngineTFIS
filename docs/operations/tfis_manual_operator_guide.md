...
| `powershell -ExecutionPolicy Bypass -File scripts\refresh_tfis_operator_dashboard.ps1` | Rebuild the operator dashboard and reuse the existing dashboard server if it is already running; start the dashboard server only when it is missing. Add `-RestartDashboardServer` when a dashboard server code fix must be loaded. | During market hours when TFIS paper runtime is already healthy and you only want refreshed dashboard HTML or a dashboard-only server restart. | Console should report dashboard build success, either reuse or start of the dashboard server, and dashboard accepting connections on `127.0.0.1:8765`. With `-RestartDashboardServer`, it should stop only the dashboard server PID(s), then start a fresh dashboard server. | Dashboard-only refresh path. Does not stop the shared lifecycle supervisor or touch `TradingEngineProd`. |
| `.\.venv\Scripts\python.exe scripts\serve_operator_dashboard.py --output-root tmp\operator_dashboard --port 8765 --skip-build` | Serve the operator dashboard locally and schedule stale dashboard rebuilds in the background on normal page requests. | After `build_operator_dashboard.py` or after the TFIS reset script already rebuilt the dashboard. | Open `http://127.0.0.1:8765/index.html`; pages show a built-at freshness strip, and stale pages schedule a background rebuild after the configured `--auto-rebuild-seconds` interval without blocking tab navigation. | Reads local artifacts only. Use `--disable-auto-rebuild` only when you intentionally want static serving with no background rebuild scheduling. |
| `powershell -ExecutionPolicy Bypass -File scripts\reset_tfis_dashboard_and_watchers.ps1 -MorningStartup` | Start TFIS as one application: build/serve the dashboard, prepare broker runtime auth once per configured provider, run configured morning strategy wrappers with refresh skipped, and start one shared lifecycle supervisor. | Normal scheduled morning startup path. The Windows task should be `TFIS Morning Startup`, enabled for weekdays at `09:08`. | Console/task logs should show provider auth preparation, dashboard URL on `127.0.0.1:8765`, configured wrappers launched sequentially, and shared supervisor launch. If FYERS temporarily returns no intraday candles for a morning snapshot window, the S21/S23 runners retry three times with a 20-second delay before recording `MARKET_CLOSED_NO_ACTION`. | Does not automatically stop existing TFIS runtime processes. The separate S21/S23 morning scheduled tasks should stay disabled to avoid auth races. |
| `powershell -ExecutionPolicy Bypass -File scripts\reset_tfis_dashboard_and_watchers.ps1` | Cleanly stop prior TFIS runtime, rebuild the dashboard once, start the dashboard server, and launch the shared lifecycle supervisor. | Standard TFIS start/recovery path after reboot, manual cleanup, or pre-market operator restart. Do not use this as an in-market dashboard refresh command. | Console should report prior runtime stop timing, dashboard build success, dashboard server URL on `127.0.0.1:8765`, dashboard accepting connections, and shared supervisor launch. | TFIS-only full reset path. During 09:15-15:30 on a trading day it refuses to stop runtime unless `-ForceInMarketReset` is passed deliberately. |
| `powershell -ExecutionPolicy Bypass -File scripts\reset_tfis_dashboard_and_watchers.ps1 -RecoverSharedSupervisor` | Start only the shared TFIS paper lifecycle supervisor after active-market safety checks. | During `09:15-15:30` on a trading day when status shows `start_shared_supervisor` and no full reset/recalculation is desired. | Console should show guardrail, waiting-order, reconciliation, and order-routing safety checks passing, then `Started shared TFIS paper lifecycle supervisor recovery PID=...`. | Does not stop runtime, rebuild dashboard, rerun strategy wrappers, or refresh auth. It refuses outside active market, if a supervisor already exists, or if safety checks fail. |
...
| `powershell -ExecutionPolicy Bypass -File scripts\show_tfis_runtime_status.ps1` | Print the current TFIS runtime process count, pause-marker state, paused strategies, shared runtime heartbeats, lifecycle audit status, waiting-order status, restart/recovery pending-action status, broker-health probe status, order-routing safety, strategy-trust evidence, runtime reconciliation status, fresh-entry handoff status, and latest operator pause/resume event from the shared control root. | During market hours when you want a quick TFIS-only health/status snapshot without restarting anything. | Console should show `GlobalPause`, `PausedStrategies`, `BrokerHealth`, `RuntimeHeartbeats`, `LifecycleAudit`, `WaitingOrders`, `RestartRecoveryStatus`, `OrderRoutingSafety`, `StrategyTrust`, `RuntimeReconciliation`, `FreshEntryHandoffs`, `LatestControlEvent`, and the current TFIS process list. `RestartRecoveryStatus` should say whether TFIS is running, needs dashboard/supervisor recovery, has stale waiting-order cleanup pending, or is stopped and ready for the morning startup path. | Read-only TFIS operator status path. Does not stop runtime or touch `TradingEngineProd`. Add `-RequireToken` when you want the broker-health probe to refresh auth prerequisites first. |
| `.\.venv\Scripts\python.exe scripts\show_tfis_live_money_boundary_status.py` | Show the explicit live-money execution/reconciliation boundary and required gates before live order routing can be considered. | During readiness review or before any discussion of enabling live-money mode. | Output should show `status=LIVE_MONEY_NO_GO_ROUTING_DISABLED`, `live_money_ready=false`, and `order_routing_enabled=false` until a separate reviewed live-routing enablement change supplies broker truth, event ingress, operator approval, kill-switch, idempotency, and reconciliation evidence. | Read-only. This does not place orders; it documents that live routing remains blocked by design. |
| `powershell -ExecutionPolicy Bypass -File scripts\check_s23_paper_order_finalizer_task.ps1` | Compatibility read-only check for the TFIS paper-order finalizer scheduled task. | After scheduler edits or before market close. | The preferred task is `TFIS Paper Order Finalizer`, enabled for weekdays at `15:35`; the old `TFIS S23 Paper Order Finalizer` task should normally be disabled. | Read-only. The script name is retained for compatibility, but the task is no longer S23-only. |
| `powershell -ExecutionPolicy Bypass -File scripts\start_s23_paper_order_finalizer.ps1 -DryRun` | Run the application-level post-cutoff paper-order finalizer without mutating order state. It reads the shared lifecycle target config and sweeps every configured paper root. | Post-market verification or when checking stale waiting orders. | Dry-run output should show per-root scanned/finalized/skipped counts. Use `-CurrentSessionOnly` only when deliberately excluding prior-session cleanup. | Does not place broker orders. Without `-DryRun`, it marks eligible waiting paper orders as `PAPER_ORDER_NOT_FILLED` through the normal paper order-state store. |
| `powershell -ExecutionPolicy Bypass -File scripts\pause_tfis_runtime.ps1` | Apply a TFIS paper-runtime pause marker so the shared supervisor stops managing all strategies or one selected strategy. Use `-StrategyCode S21` or `-StrategyCode S23` for strategy scope; omit it for a global pause. | During market hours when an operator wants TFIS supervision to fail closed temporarily without killing the whole runtime. | Console should report the pause scope and the marker file path under `tmp\operator_controls`. The shared supervisor should then skip paused strategies on the next loop. | Pause affects TFIS paper supervision only. It does not touch `TradingEngineProd`. The command also appends an audit row to `tmp\operator_controls\operator_control_events.jsonl`. |
| `powershell -ExecutionPolicy Bypass -File scripts\resume_tfis_runtime.ps1` | Clear a previously applied TFIS paper-runtime pause marker globally or for one selected strategy. | After a temporary operator pause when supervision may resume safely. | Console should report the cleared scope and marker file path. The shared supervisor should resume managing that scope on the next loop. | Resume affects TFIS paper supervision only. It does not touch `TradingEngineProd`. The command also appends an audit row to `tmp\operator_controls\operator_control_events.jsonl`. |
